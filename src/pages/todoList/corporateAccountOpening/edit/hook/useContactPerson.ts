...
import { addContactPerson, updateContactPerson } from "@/features/contactPersonSlice";
...
export function useContactPerson() {
  ...
  const saveContactPerson = async (data: TContactPersonArray) => {
    ...
    try {
      ...
      if (data.contacts[0].personalId) { //ถ้าส่งแบบมี personalId แปลว่าเป็นการ update
        ...
        if (res.status === 200) {
          ...
          dispatch(addContactPerson({ ...data.contacts[0], personalId: res.data.personalId }));
        } else {
