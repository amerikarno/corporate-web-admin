        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-4 pt-8">
              <div className="flex space-x-4">
                  <div className="flex w-1/2">
                    <select  {...register("thTitle")} className="p-1  border-b-2 border-slate-700 
              ...
              <div className="flex space-x-4">
                <div className="w-1/2">
                  <Input type="text" {...register("thName")} label="ชื่อ (ภาษาไทย)" id="thName"/>
                  {errors.thName && <span>{errors.thName.message}</span>}
                </div>
                <div className="w-1/2">
                  <Input type="text" {...register("thSurname")} label="ชื่อสกุล (ภาษาไทย)" id="thSurname"/>
            ...
            <div className="space-y-4">
              <div className="flex space-x-4">
                <div className="w-1/2">
                  <select  {...register("engTitle")} className="p-1  border-b-2 border-slate-700 
                    text-gray-900 text-sm block w-full hover:border-b-4 transition-all cursor-pointer">
              ...
              <div className="flex space-x-4">
                <div className="w-1/2">
                  <Input type="text" {...register("engName")} label="คำนำหน้าชื่อ (ภาษาอังกฤษ)" id="engName"/>
            ...
            <div className="flex space-x-4 pt-8">
              <div className="w-1/2">
                <Input type="email" {...register("email")} label="อีเมลล์" id="email"/>
                {errors.email && <span>{errors.email.message}</span>}
              </div>

              <div className="w-1/2">
                <Input type="text" {...register("mobile")} label="หมายเลขโทรศัพท์มือถือ" id="mobile"/>
            ...
            <div className="flex space-x-4">
              <div className="w-1/2">
                <Input type="date" {...register("birthDate")} label="วัน/เดือน/ปี เกิด" />
                {errors.birthDate && <span>{errors.birthDate.message}</span>}
              </div>

              <div className="w-1/2">
                <select  {...register("mariageStatus")} className="cursor-pointer border border-gray-700 text-gray-600 pl-2 hover:bg-slate-100
            ...
            <div className="flex flex-col space-y-2">
              ...
              <input
                ...
                />
                <span className="text-gray-500">ข้อพเจ้าได้อ่านและตกลงตามข้อกำหนดและเงื่อนไขและรับทราบนโยบายความเป็นส่วนตัวซึ่งระบุวิธีการที่บริษัท ฟินันเซียดิจิทัล แอสแซท จำกัด("บริษัท")</span>
              </div>
              <div>
                <span className="text-red-500 cursor-pointer">อ่านรายละเอียดเพิ่มเติม</span>
              </div>
            </div>
