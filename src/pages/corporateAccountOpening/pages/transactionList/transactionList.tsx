...
const TransactionList = () => {
    ...
    const handleCheckboxChange = (transactionId: string, type: 'approve' | 'decline', checked: boolean) => {
        setCheckboxStatus((prevStatus) => {
            ...
            let newStatus;
            if (type === 'approve') {
                newStatus = checked ? true : (currentStatus === true ? undefined : currentStatus);
            } else if (type === 'decline') {
                newStatus = checked ? false : (currentStatus === false ? undefined : currentStatus);
            }
    ...
    const handleSubmit = () => {
        const dataToSend = listOfTransaction.map((transaction) => ({
            transactionId: transaction.id,
            checkerStatus: checkboxStatus[transaction.id]?.checkerStatus === true
                ? true
        ...
        axios.post('/api/v1/transaction/order/update', dataToSend, {
            headers: {
      ...
      const columnsContactPerson: TableColumn<TTransaction>[] = [
          ...
          },
        {
          ...
          cell: (row: TTransaction) => (
            ...
            onChange={(e) => handleCheckboxChange(row.id, 'approve', e.target.checked)}
            />
          ),
          ignoreRowClick: true,
        },
        {
          ...
          cell: (row: TTransaction) => (
            ...
            onChange={(e) => handleCheckboxChange(row.id, 'decline', e.target.checked)}
            />
          ),
          ignoreRowClick: true,
        },
