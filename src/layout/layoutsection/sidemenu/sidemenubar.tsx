...
import MenuItems, { sidemenuType } from "../../../common/sidemenudata";

let history: string[] = [];

const Sidebar = () => {

	const [menuitems, setMenuitems] = useState<sidemenuType[]>(MenuItems);
	...
	// const _menuIcontype = document.querySelector("body")?.classList.contains("horizontalmenu") ? "hor-icon" : "sidemenu-icon";
	...
	function Clickhandelar() {
		...
		if (document.querySelector("html")?.getAttribute("data-vertical-style") == "doublemenu") {
			document.querySelector("html")?.setAttribute("toggled", document.querySelector("html")?.getAttribute("toggled") == 'double-menu-open' ? "" : "double-menu-open")
		}
	}
	...
	let slideRight = () => {
		...
		if (menuNav && mainContainer1) {
			...
			let element1: any = document.querySelector(".main-menu > .slide.open >ul")
			if (element) {
									...
									{firstlayer.Items.map((secondlayer: any) =>
										secondlayer.type === "link" ?
											<li className={`slide ${secondlayer.active ? "active" : ''}`} key={Math.random()} >
												...
												<Link onClick={(_event) => { setSidemenu(secondlayer); }} to={secondlayer.path + "/"} className={`side-menu__item ${secondlayer.selected ? "active" : ""}`}>{secondlayer.icon} <span className="side-menu__label">{secondlayer.title}</span></Link>
											</li>
											: secondlayer.type === "sub" ?
												<li onClick={() => { Clickhandelar() }} className={`slide has-sub ${secondlayer.active ? "open" : ''}`} key={Math.random()}>
													<Link to="#" className={`side-menu__item ${secondlayer.selected ? "active" : ""}`}
														onClick={(event) => { event.preventDefault(); toggleSidemenu(secondlayer) }}>
														{secondlayer.icon}
														<span className="side-menu__label">{secondlayer.title}</span>
														<i className="ri ri-arrow-right-s-line side-menu__angle"></i></Link>
														...
														{secondlayer.children.map((thirdlayer: any) =>
															<Fragment key={Math.random()} >
																{thirdlayer.type === "link" ? <li className={`slide ${thirdlayer.active ? "active" : ''}`}>
																	<Link onClick={(_event) => { setSidemenu(thirdlayer); }} to={thirdlayer.path + "/"} className={`side-menu__item ${thirdlayer.selected ? "active" : ''}`}>
																		{thirdlayer.icon}
																		{thirdlayer.title}
																	</Link></li>
																	: thirdlayer.type === "sub" ? <li className={`slide has-sub ${secondlayer.active ? "open" : ''}`}>
																		<Link to="#" className="side-menu__item" onClick={(evnt) => {
																			evnt.preventDefault();
																			toggleSidemenu(thirdlayer)
																		}}>{thirdlayer.icon} <span className="">
																				{thirdlayer.title}
																			</span><i className="ri ri-arrow-right-s-line side-menu__angle"></i></Link>
																		<ul className={`slide-menu child2 ${thirdlayer.active ? "!block" : "" }`}>
																			{thirdlayer.children.map((fourthlayer: any) => <Fragment key={Math.random()}>
																				{fourthlayer.type === "link" ?
																					<li className={`slide ${fourthlayer.active ? "active" : ''}`}>
																						<Link onClick={(_event) => { setSidemenu(fourthlayer); }} to={fourthlayer.path + "/"} className={`side-menu__item ${fourthlayer.selected ? "active" : ''}`}>
																							{fourthlayer.icon}
																							{fourthlayer.title}
																						</Link></li>
																					: fourthlayer.type === "sub" ? <li className={`slide has-sub ${fourthlayer.active ? "open" : ''}`}>
																						...
																						}}>{fourthlayer.icon} <span className="">{fourthlayer.title}</span><i className="ri ri-arrow-right-s-line side-menu__angle"></i></Link>

																						<ul className={`slide-menu child3 ${fourthlayer.active ? "!block" : ""
																							}`}>
																							{fourthlayer.children.map((fivthlayer: any) => <Fragment key={Math.random()}>
																								{fivthlayer.type === "link" ?
																									<li className="slide"><Link to={fourthlayer.path + "/"} className="side-menu__item">
																										{fivthlayer.icon} {fivthlayer.title}</Link></li> : <Link to="#" className="side-menu__item" onClick={(evnt) => {
																											evnt.preventDefault();
																											toggleSidemenu(fivthlayer)

																										}}>{fivthlayer.icon} <span className="">{fivthlayer.title}</span><i className="ri ri-arrow-right-s-line side-menu__angle"></i></Link>
																								}
																							</Fragment>)}
