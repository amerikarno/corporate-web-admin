...
const Dropbox: React.FC<DropboxProps> = ({ onDropdownSelect }) => {
  ...
  <div className="relative">
    ...
    <div id="dropdown" className={`z-40 ${isDropDown ? ' ' : 'hidden'} max-w-66 absolute bg-neutral-200 divide-y divide-gray-100 rounded-lg shadow w-full dark:bg-gray-700`}>
        <ul className="border-black border-2 py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
        <li onClick={()=>handleSelect("IDCard")}>
            <a className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">CardID</a>
        </li>
        <li onClick={()=>handleSelect("Passport")}>
            <a className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Passport</a>
        </li>
        </ul>
    </div>
</div>  
