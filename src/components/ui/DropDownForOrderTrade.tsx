...
export function DropDownForOderTrade<T extends Record<string, any>>({
  onDropdownSelect,
  items,
  className,
  ...props
  ...
  const [dropDownChoosed, setDropDownChoosed] = useState<string>("Currency");
        ...
        >
          ...
          {items.map((item, index) => (
            <li key={index} onClick={() => handleSelect(item)}>
              <a className="font-bold block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                {item.value}
              </a>
            </li>
          ))}
